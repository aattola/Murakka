...
export class CommandErrorListener extends Listener<typeof Events.ChatInputCommandError> {
  ...
  private generateUnexpectedErrorMessage(interaction: CommandInteraction, error: any) {
    if (interaction.user.id === '214760917810937856') return codeBlock('js', error.stack!)
    return `Hei nyt tapahtui yllättävä virhe voitko ilmoitella tästä eteenpäin?`
  }

  private reply(interaction: CommandInteraction, content: string, ephemeral = true) {
  ...
  public override async run(error: any, { command, interaction }: ChatInputCommandErrorPayload) {
